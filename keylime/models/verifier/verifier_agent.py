...
class VerifierAgent(PersistableModel):
    ...
    def get_pcr_policy(self, pcr_bank):
        policy = {}
        pcr_size = algorithms.Hash(pcr_bank).get_size()

        for pcr_num, allowed_vals in self.tpm_policy.items():
    ...
    @property
    def allowable_pcr_banks(self):
        if not self.tpm_policy:
            return self.accept_tpm_hash_algs

        allowable_pcr_banks = []

        for alg in self.accept_tpm_hash_algs:
            policy = self.get_pcr_policy(alg)

            pcrs_with_allowlist = [ pcr_num for pcr_num, allowed_vals in policy.items() if allowed_vals ]
            
            if pcrs_with_allowlist:
                allowable_pcr_banks.append(alg)
        
        if not allowable_pcr_banks:
            return self.accept_tpm_hash_algs

        return allowable_pcr_banks
