...
from keylime import config, json, keylime_logging, secure_mount
...
def post_cfssl(params, data):
    ...
    while True:
        try:
            response = requests.post(f"http://{cfssl_ip}:{cfssl_port}/{params}", json=data, timeout=1)
            break
...
def start_cfssl(cmdline=""):
    ...
    cfsslproc = subprocess.Popen(  # pylint: disable=consider-using-with
        cmd,
        env=env,
        shell=True,
...
def mk_cacert(name=None):
    ...
    csr = {
        "CN": name,
        "key": {"algo": "rsa", "size": config.getint("ca", "cert_bits")},
        "names": [
            {
...
def gencrl(serials, cert, ca_pk):
    request = {"certificate": cert, "serialNumber": serials, "issuingKey": ca_pk, "expireTime": ""}
    secdir = secure_mount.mount()
    try:
        # need to temporarily write out the private key with no password
        # to tmpfs
    ...
    if body["success"]:
        ...
        raise Exception(f"Unable to create crl for cert serials {serials}. " f"Error: {body['errors']}")
    return retval
