...
.. http:get::  /v2.5/agent/info
    ...
    **Example response**:
    ...
    :>json string tpm_enc_alg: The encryption algorithm used by this agent's TPM device. In API v2.5+, this includes explicit bit-length (e.g., ``rsa2048``, ``ecc256``). Generic formats (``rsa``, ``ecc``) from earlier API versions are normalized to their default bit-lengths.
    :>json string tpm_sign_alg: The signing algorithm used by this agent's TPM device.
    :>json string ak_handle: The Attestation Key handle of the TPM device used by this agent.

.. http:get::  /v2.5/keys/pubkey
...
.. http:get::  /v2.5/quotes/integrity
    ...
    **Example Response:**
    ...
    :>json string hash_alg: Hash algorithm used in the quote. Supported values: ``sha1``, ``sha256``, ``sha384``, ``sha512``, ``sm3_256``.
    :>json string enc_alg: Encryption algorithm used in the quote. In API v2.5+, this includes explicit bit-length specifications. Supported values: ``rsa``, ``rsa1024``, ``rsa2048``, ``rsa3072``, ``rsa4096``, ``ecc``, ``ecc192``, ``ecc224``, ``ecc256``, ``ecc384``, ``ecc521``, ``ecc_sm2``. Generic formats (``rsa``, ``ecc``) are normalized to ``rsa2048`` and ``ecc256`` respectively.
    :>json string sign_alg: Signing algorithm used in the quote. Supported values: ``rsassa``, ``rsapss``, ``ecdsa``, ``ecdaa``, ``ecschnorr``.
