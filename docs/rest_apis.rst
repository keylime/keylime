...
~~~~~~~~~~~~~~~~~~~~
This CA is by created by verifier on startup.
It contains the server certificates and keys used by the verifier and registrar for their respective HTTPS interface.
Then it also contains the client certificates and keys that is used by the tenant to connect to the registrar, verifier
and agent. Also the verifier uses that certificate to authenticate itself against the agent.

Agent Keylime CA
~~~~~~~~~~~~~~~~
The agent runs a HTTPS server and provides it's certificate to the registrar (:code:`mtls_cert`).
