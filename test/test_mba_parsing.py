...
try:
    ...
    TPM2_ELPARSER_AVAILABLE = True
except Exception:
    tpm2_tools_elparser = None  # type: ignore[assignment]
    TPM2_ELPARSER_AVAILABLE = False
...
class TestMBAParsing(unittest.TestCase):
    def test_parse_bootlog(self) -> None:
    ...
    def test_vendor_db_enrichment_actual_hex(self) -> None:
    ...
    def test_vendor_db_enrichment_multiple_certificates_real_data(self) -> None:
    ...
    def test_vendor_db_enrichment_preserves_signature_lists(self) -> None:
...
@unittest.skipUnless(TPM2_ELPARSER_AVAILABLE, "tpm2-tools not installed")
class TestParseBootlogTpm2Eventlog(unittest.TestCase):
    """Unit tests for parse_binary_bootlog focusing on tpm2_eventlog exit-code handling."""

    def test_nonzero_exit_code_returns_failure(self) -> None:
        """tpm2_eventlog non-zero exit code must produce a failure and no result."""
        mock_retdict = {"retout": [], "reterr": [b"fatal error"], "code": 1}
        with mock.patch("keylime.cmd_exec.run", return_value=mock_retdict):
            failure, result = tpm2_tools_elparser.parse_binary_bootlog(b"\x00" * 8)  # type: ignore[union-attr]
        self.assertIsNone(result)
        self.assertTrue(failure)

    def test_stderr_with_zero_exit_code_logs_warning(self) -> None:
        """tpm2_eventlog stderr with exit code 0 must log a warning and not fail early."""
        mock_retdict = {
            "retout": [b"some: yaml\n"],
