...
class TestSessionManagerCleanup(unittest.TestCase):
    """Test SessionManager cleanup() method for proper resource cleanup."""

    def test_cleanup_calls_remove_on_scoped_session(self):
        """Verify cleanup() calls remove() on scoped_session when it exists."""
        from keylime.db.keylime_db import SessionManager  # pylint: disable=import-outside-toplevel

        sm = SessionManager()
        # Create a mock scoped_session
    ...
    def test_cleanup_handles_none_scoped_session(self):
        """Verify cleanup() safely handles None scoped_session without errors."""
        from keylime.db.keylime_db import SessionManager  # pylint: disable=import-outside-toplevel

        sm = SessionManager()
        # Ensure _scoped_session is None
        sm._scoped_session = None  # pylint: disable=protected-access

        # Call cleanup - should not raise any exceptions
        try:
            sm.cleanup()
        except Exception as e:  # pylint: disable=broad-except
            self.fail(f"cleanup() raised exception with None scoped_session: {e}")

    def test_session_context_calls_cleanup_in_finally(self):
        """Verify session_context() calls cleanup() in finally block."""
        from keylime.db.keylime_db import SessionManager  # pylint: disable=import-outside-toplevel
        ...
        # Verify cleanup was called (which calls remove())
        mock_scoped_session.remove.assert_called()

    def test_session_context_calls_cleanup_even_on_exception(self):
        """Verify session_context() calls cleanup() even when exception occurs."""
        from keylime.db.keylime_db import SessionManager  # pylint: disable=import-outside-toplevel
